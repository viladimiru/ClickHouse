...
class Runner:
            ...
            if exit_code != 0:
                if not result.is_completed():
                    if process.timeout_exceeded:
                        ...
                        print(f"ERROR: Invalid status [{result.status}] for exit code [{exit_code}]  - switch to [{Result.Status.ERROR}]")
                        result.set_status(Result.Status.ERROR).set_info(ResultInfo.INVALID_RESULT_STATUS)
            result.dump()
    ...
    def run(self, workflow, job, docker="", dummy_env=False, no_docker=False):
        res = True
        setup_env_code = -10
        prerun_code = -10
        run_code = -10
        ...
        if res:
            ...
            try:
                run_code = self._run(workflow, job, docker=docker, no_docker=no_docker)
                res = run_code == 0
                if not res:
